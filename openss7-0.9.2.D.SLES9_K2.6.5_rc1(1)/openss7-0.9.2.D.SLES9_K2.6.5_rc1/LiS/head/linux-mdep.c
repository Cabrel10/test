/*****************************************************************************
 ...
 @(#) linux-mdep.c,v OpenSS7-0_9_2_D_rc2(1.1.1.11.4.24) 2006/07/07 21:21:46
 ...
 Last Modified 2006/07/07 21:21:46 by brian

 -----------------------------------------------------------------------------

 linux-mdep.c,v
 Revision 1.1.1.11.4.24  2006/07/07 21:21:46  brian
...
#ident "@(#) linux-mdep.c,v OpenSS7-0_9_2_D_rc2(1.1.1.11.4.24) 2006/07/07 21:21:46"
 ...
 * RCS Id          : linux-mdep.c,v 1.1.1.11.4.24 2006/07/07 21:21:46 brian Exp
...
************************************************************************/
void
lis_bug(const char *file, unsigned int line, const char *fmt, ...)
{
	static char buf[256];
	va_list args;
 ...
 */
# if defined(KERNEL_2_5)
struct dentry *lis_inode_lookup(struct inode *dir, struct dentry *dentry, struct nameidata *nd);
# else
...
struct vfsmount *
lis_mntget_fcn(struct vfsmount *m, const char *file, int line, const char *fn)
{
	...
	if (LIS_DEBUG_REFCNTS) {
		if (mm == NULL)
			...
			printk("lis_mntget(m@0x%p/++%d) \"%s\"%s {%s@%d,%s()}\n", mm, MNT_COUNT(mm),
			       mm->mnt_devname, (lis_mnt
						 && mm == lis_mnt ? " <lis_mnt>" : ""), file, line,
			       fn);
	}
...
void
lis_mntput_fcn(struct vfsmount *m, const char *file, int line, const char *fn)
{
	if (m == NULL) {
		if (LIS_DEBUG_REFCNTS)
	...
	if (LIS_DEBUG_REFCNTS)
		printk("lis_mntput(m@0x%p/%d%s) \"%s\"%s {%s@%d,%s()}\n", m, MNT_COUNT(m),
		       (MNT_COUNT(m) > 0 ? "--" : ""), m->mnt_devname, (lis_mnt
									&& m ==
									lis_mnt ? " <lis_mnt>" :
									""), file, line, fn);
...
static
    void
lis_modsync_dbg(const char *file, int line, const char *fn)
{
#ifdef THIS_MODULE
...
void
lis_modget_dbg(const char *file, int line, const char *fn)
{
#ifdef THIS_MODULE
	if (LIS_DEBUG_REFCNTS)
		printk("lis_modget() <\"%s\"/%ld>++ {%s@%d,%s()}\n", (THIS_MODULE)->name,
		       lis_modcnt(THIS_MODULE), BFN(file), line, fn);
...
void
lis_modput_dbg(const char *file, int line, const char *fn)
{
#ifdef THIS_MODULE
	long cnt;
 ...
 */
static struct inode *
lis_get_new_inode(struct super_block *sb)
{
	struct inode *i = NULL;;
	...
	if (LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS)
		printk("lis_get_new_inode(s@0x%p)%s%s >> i@0x%p/%d%s\n", sb,
		       (S_IS_LIS(sb) ? " <LiS>" : ""), (lis_mnt
							&& sb ==
							lis_mnt->mnt_sb ? " <lis_mnt>" : ""), i,
		       (i ? I_COUNT(i) : 0), (i && I_IS_LIS(i) ? " <LiS>" : ""));
	return (i);
}
...
************************************************************************/
long
lis_time_till(long target_time)
{
...
************************************************************************/
long
lis_target_time(long milli_sec)
{
	if (((1000 / HZ) * HZ) == 1000)	/* ie: HZ == 100 */
...
struct dentry *
lis_d_alloc_root(struct inode *i, int mode)
{
#if defined(FATTACH_VIA_MOUNT)
...
lis_cdev_put(struct dentry *d)
{
	...
	if (LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) {
		...
		if (inode && inode->i_cdev)
			printk(">> i->i_cdev: c@0x%p/%d/%x \"%s\"\n", inode->i_cdev,
			       ...
			       (inode->i_cdev->owner ? inode->i_cdev->owner->name : "No-Owner"));
	}
	...
	spin_lock(&lock);
	if (D_COUNT(d) == 1 && I_COUNT(inode) == 1 && !list_empty(&inode->i_devices)) {
		inode->i_cdev = NULL;
...
int
lis_select(struct inode *inode, struct file *file, int sel_type, select_table * wait)
{
	stdata_t *hd;
	polldat_t pdat;
	int evts;
...
#if !defined(LINUX_POLL)
void
lis_select_wakeup(stdata_t *hd)
{
	CLR_SD_FLAG(hd, STRSELPND);	/* no longer waiting on select */
	if (hd == NULL || hd->magic != STDATA_MAGIC || hd->sd_select.sel_wait == NULL
...
void
lis_show_inode_aliases(struct inode *i)
{
#if defined(KERNEL_2_1)
	struct list_head *ent;

	if (!(LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) || list_empty(&(i->i_dentry)))
		return;

	printk("lis_show_inode_aliases(i@0x%p/%d.%d#%lu)%s:\n", i, I_COUNT(i), i->i_nlink, i->i_ino,
	       (I_IS_LIS(i) ? " <LiS>" : ""));
 ...
 */
char *
lis_alloc_file_path(void)
{
	return (char *) __get_free_page(GFP_USER);
}

char *
lis_format_file_path(struct file *f, char *page)
{
	if (page) {
#if defined(KERNEL_2_5)
		char *path = d_path(f->f_dentry, FILE_MNT(f), page, PAGE_SIZE);
#else
		char *path = __d_path(f->f_dentry, FILE_MNT(f), FILE_MNT(f)->mnt_root, NULL, page,
				      PAGE_SIZE);
#endif
		return path;
	} else
...
void
lis_print_file_path(struct file *f)
{
 ...
 */
void
lis_print_dentry(struct dentry *d, char *comment)
{
	char dname[100];
	...
	printk("%s: d@0x%p/%d%s m:%d", comment, d, D_COUNT(d), (D_IS_LIS(d) ? " <LiS>" : ""),
	       K_ATOMIC_READ(&lis_mnt_cnt));
	if (i) {
		printk(" i@0x%p/%d%s", i, I_COUNT(i), (I_IS_LIS(i) ? " <LiS>" : ""));
#if defined(KERNEL_2_5)
		if (i->i_cdev)
...
static
    int
lis_fs_fattach_sb(struct super_block *sb, void *ptr, int silent)
{
#if defined(FATTACH_VIA_MOUNT)
	...
	if (file && head && head == FILE_STR(file) && head->magic == STDATA_MAGIC && head->sd_inode) {
		/* 
		...
		if (LIS_DEBUG_FATTACH || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) {
			printk("lis_fs_fattach_sb(s@0x%p,@0x%p,...) << [%d]"
			       ...
			       K_ATOMIC_READ(&lis_mnt_cnt), file, (file ? F_COUNT(file) : 0),
			       FILE_MNT(file), (FILE_MNT(file) ? MNT_COUNT(FILE_MNT(file)) : 0),
			       (S_IS_LIS(sb) ? " <LiS>" : ""), (file
								&& FILE_MNT(file) ==
								lis_mnt ? " <lis_mnt>" : ""));
		}
		...
		sb->s_root = d_mount = lis_d_alloc_root(igrab(i_mount), LIS_D_ALLOC_ROOT_MOUNT);
		if (sb->s_root == NULL) {
			ULOCK_INO(i_mount);
			lis_head_put(head);
			return (-ENOMEM);
		...
		if ((LIS_DEBUG_FATTACH || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) && path) {
			dev_t dev = GET_I_RDEV(i_mount);

			printk("lis_fs_fattach_sb(s@0x%p,@0x%p,%d) "
			       ">> d@0x%p/%d i@0x%p/%d rdev (%d,%d)\n", sb, data, silent, d_mount,
			       D_COUNT(d_mount), i_mount, I_COUNT(i_mount), getmajor(dev),
			       getminor(dev));
			printk("lis_fs_fattach_sb(s@0x%p,@0x%p,...)\n"
			       "    >> f@0x%p/%d h@0x%p/%d/%d \"%s\""
			       " sb@0x%p d@0x%p/%d (i@0x%p/%d)\n", sb, data, file, F_COUNT(file),
			       head, LIS_SD_REFCNT(head), LIS_SD_OPENCNT(head), path, data->sb,
			       d_mount, D_COUNT(d_mount), i_mount, I_COUNT(i_mount));
			lis_show_inode_aliases(i_mount);
		}

		if ((LIS_DEBUG_FATTACH || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) && file) {
			printk("lis_fs_fattach_sb(s@0x%p,@0x%p,...) >> [%d]"
			       " f@0x%p/%d f_vfsmnt 0x%p/%d%s%s\n", sb, data,
			       K_ATOMIC_READ(&lis_mnt_cnt), file, F_COUNT(file), FILE_MNT(file),
...
static
    int
lis_fs_kern_mount_sb(struct super_block *sb, void *ptr, int silent)
{
	...
	if (LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS)
		printk("lis_fs_kern_mount_sb(s@x%p,...)"
		       ...
		       D_COUNT(sb->s_root), isb, I_COUNT(isb), getmajor(GET_I_RDEV(isb)),
...
int
lis_fs_setup_sb(struct super_block *sb, void *ptr, int silent)
{
...
#else				/* 2.4 kernel */
struct super_block *
lis_fs_read_super(struct super_block *sb, void *ptr, int silent)
{
...
************************************************************************/
int
lis_dentry_delete(struct dentry *d)
{
	if (LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS)
		printk("lis_dentry_delete(d@0x%p/%d) [%d]%s\n", d, D_COUNT(d),
		       K_ATOMIC_READ(&lis_mnt_cnt), (D_IS_LIS(d) ? " d<LiS>" : ""));

	return (1);
}

void
lis_dentry_iput(struct dentry *d, struct inode *i)
{
	if (LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS)
...
************************************************************************/
void
lis_super_put_inode(struct inode *i)
{
	MNTSYNC();

	if (LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) {
		printk("lis_super_put_inode(i@0x%p/%d)%s " "i_rdev=0x%x <[%d] %d LiS inode(s)>%s\n",
		       i, I_COUNT(i), (I_IS_LIS(i) ? " <LiS>" : ""), GET_I_RDEV(i),
 ...
 */
void
lis_super_umount_begin(struct super_block *sb)
{
	...
	if (LIS_DEBUG_FATTACH || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS)
		printk("lis_super_umount_begin(s@0x%p) << data@0x%p:\n"
		       "    f@0x%p m@0x%p/%d h@0x%p/%d/%d" " d@0x%p/%d (i@0x%p/%d)\n", sb, data,
		       file, mount, (mount ? MNT_COUNT(mount) : 0), head,
		       (head ? LIS_SD_REFCNT(head) : 0), (head ? LIS_SD_OPENCNT(head) : 0),
		       d_umount, (d_umount ? D_COUNT(d_umount) : 0), i_umount,
		       (i_umount ? I_COUNT(i_umount) : 0));
	...
	if (data && head && d_umount && i_umount) {
		...
		if (LIS_DEBUG_FATTACH || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) {
			printk("lis_super_umount_begin(%p) [fdetach] "
			       "i@0x%p/%d d@0x%p/%d \"%s\"\n", sb, i_umount, I_COUNT(i_umount),
			       d_umount, D_COUNT(d_umount), data->path);
		...
		if (LIS_DEBUG_FATTACH || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS)
			printk("lis_super_umount_begin(s@0x%p) [fdetach] "
			       ...
			       (d_umount ? D_COUNT(d_umount) : 0), i_umount,
			       (i_umount ? I_COUNT(i_umount) : 0));
	}
...
void
lis_super_put_super(struct super_block *sb)
{
	lis_fattach_t *data = (lis_fattach_t *) S_FS_INFO(sb);
	struct dentry *d_umount = (data ? data->dentry : NULL);
	struct inode *i_umount = (d_umount ? d_umount->d_inode : NULL);
	...
	if (d_umount && i_umount) {
		if (LIS_DEBUG_FATTACH || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS)
			printk("lis_super_put_super(s@0x%p) [fdetach] " "d@0x%p/%d (i@0x%p/%d)\n",
			       sb, d_umount, D_COUNT(d_umount), i_umount,
			       (i_umount ? I_COUNT(i_umount) : 0));
...
int
lis_new_file_name_dev(struct file *f, const char *name, dev_t dev)
{
	struct qstr dname;
	struct dentry *new;
	...
	if (LIS_DEBUG_VOPEN || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) {
		...
		printk("lis_new_file_name_dev(f@0x%p/%d,\"%s\",0x%x)%s", f, (f ? F_COUNT(f) : 0),
		       (name ? name : ""), dev, (FILE_MNT(f) == lis_mnt ? " <lis_mnt>" : ""));
		printk(" \"");
		if (FILE_MNT(f))
			lis_print_file_path(f);
		printk("\"\n");
		lis_print_dentry(d, ">> dentry");
	}
	...
	if (dev == 0) {		/* must use old inode */
		if (old != NULL)
			...
			return (-EINVAL);
	} else if ((oldi = lis_get_inode(S_IFCHR | S_IRUSR | S_IWUSR, dev)) == NULL)
		return (-ENOMEM);
	else
 ...
 */
void
lis_new_stream_name(struct stdata *head, struct file *f)
{
	if (LIS_DEBUG_VOPEN || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) {
		printk("lis_new_stream_name(h@0x%p/%d/%d,f@0x%p/%d)\n", head, LIS_SD_REFCNT(head),
		       LIS_SD_OPENCNT(head), f, F_COUNT(f));
		lis_print_dentry(f->f_dentry, ">> dentry");
	}

	sprintf(head->sd_name, "%s%s", lis_strm_name(head), lis_maj_min_name(head));
...
lis_new_inode(struct file *f, dev_t dev)
{
	...
	if (!old) {		/* param checking */
		printk("lis_new_inode(f@0x%p/%d,d0x%x) - " "old inode must be non-NULL\n", f,
		       F_COUNT(f), DEV_TO_INT(dev));
		return (NULL);	/* bad return */
	}

	if (lis_mnt == NULL) {
		printk("lis_new_inode(f@0x%p/%d,d0x%x) - " "LiS has been unmounted\n", f,
		       F_COUNT(f), DEV_TO_INT(dev));
		return (NULL);	/* bad return */
	}

	if (LIS_DEBUG_VOPEN || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) {
		printk("lis_new_inode(f@0x%p/%d,dv0x%x) %s%s", f, F_COUNT(f), DEV_TO_INT(dev),
	...
	if (hd == NULL) {
		printk("lis_new_inode(f@0x%p/%d,d0x%x) - " "no STREAM head\n", f, F_COUNT(f),
		       DEV_TO_INT(dev));
		return (NULL);	/* bad return */
	}
	...
	if (LIS_DEBUG_VOPEN || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) {
		printk("lis_new_inode(f@0x%p/%d,d0x%x)%s%s", f, F_COUNT(f), DEV_TO_INT(dev),
		       (D_IS_LIS(f->f_dentry) ? " <LiS>" : ""), (f
								 && FILE_MNT(f) ==
								 lis_mnt ? " <lis_mnt>" : "")
		    );
		printk(" \"");
 ...
 */
void
lis_cleanup_file_opening(struct file *f, stdata_t *head, int open_fail, struct dentry *oldd,
			 int oldd_cnt, struct vfsmount *oldmnt, int oldmnt_cnt)
{
	if (LIS_DEBUG_VOPEN || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) {
		printk("lis_cleanup_file_opening(f@0x%p/%d,h@0x%p/%d/%d,%d) %s\n", f,
		       (f ? F_COUNT(f) : 0), head, (head ? LIS_SD_REFCNT(head) : 0),
		       (head ? LIS_SD_OPENCNT(head) : 0), open_fail,
		       (FILE_MNT(f) == lis_mnt ? " <lis_mnt>" : ""));
	...
	if (open_fail) {
		...
		if (LIS_DEBUG_VOPEN || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS)
			printk("    error %d >> oldmnt@0x%p/%d %c oldmnt_cnt %d\n", open_fail,
			       ...
			       (MNT_COUNT(oldmnt) < oldmnt_cnt ? '<' : MNT_COUNT(oldmnt) >
				oldmnt_cnt ? '>' : '='), oldmnt_cnt);
		if (MNT_COUNT(oldmnt) < oldmnt_cnt)
			(void) MNTGET(oldmnt);
		else if (MNT_COUNT(oldmnt) > oldmnt_cnt)
			MNTPUT(oldmnt);
	} else {
		/* 
...
void
lis_cleanup_file_closing(struct file *f, stdata_t *head)
{
	/* 
	...
	if (LIS_DEBUG_VCLOSE || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) {
		printk("lis_cleanup_file_closing(f@0x%p/%d,h@0x%p/%d/%d)" " [%d]%s%s", f,
		       (f ? F_COUNT(f) : 0), head, (head ? LIS_SD_REFCNT(head) : 0),
		       (head ? LIS_SD_OPENCNT(head) : 0), K_ATOMIC_READ(&lis_mnt_cnt),
		       (D_IS_LIS(f->f_dentry) ? " <LiS>" : ""),
		       (FILE_MNT(f) == lis_mnt ? " <lis_mnt>" : "")
		    );
		printk(" \"");
...
struct dentry *
lis_inode_lookup(struct inode *dir, struct dentry *dentry, struct nameidata *nd)
{
	return (NULL);
}

void
lis_drop_inode(struct inode *inode)
{
...
struct dentry *
lis_inode_lookup(struct inode *dir, struct dentry *dentry)
{
	return (NULL);
}
 ...
 */
struct inode *
lis_set_up_inode(struct file *f, struct inode *inode)
{
	struct inode *new;
 ...
 */
int
lis_is_stream_inode(struct inode *i)
{
	return (I_IS_LIS(i));
}
 ...
 */
		...
		if (LIS_DEBUG_VOPEN || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS)
			printk("lis_get_inode(m0x%x,dv0x%x) >> i@0x%p/%d" " <[%d] %d LiS inodes>\n",
			       mode, dev, i, I_COUNT(i), K_ATOMIC_READ(&lis_mnt_cnt),
...
lis_old_inode(struct file *f, struct inode *i)
{
	...
	if (LIS_DEBUG_VOPEN || LIS_DEBUG_ADDRS)
		printk("lis_old_inode(f@0x%p/%d,i@0x%p/%d)%s << "
		       "i@0x%p/%d%s (dev 0x%x -> 0x%x)\n", f, F_COUNT(f), i, I_COUNT(i),
		       (I_IS_LIS(FILE_INODE(f)) ? " <LiS>" : ""), FILE_INODE(f),
		       I_COUNT(FILE_INODE(f)), (I_IS_LIS(i) ? " <LiS>" : ""),
		       GET_I_RDEV(FILE_INODE(f)), GET_I_RDEV(i));
...
struct inode *
lis_grab_inode(struct inode *ino)
{
	return (igrab(ino));
}

void
lis_put_inode(struct inode *ino)
{
	iput(ino);
}
...
lis_get_pipe(struct file **f0, struct file **f1)
{
	 ...
	 */
	if ((error = lis_get_fifo(f0)) < 0)
		goto no_fifos;
	if ((error = lis_get_fifo(f1)) < 0)
		goto no_fifo1;
...
lis_pipe(unsigned int *fd)
{
	...
	if ((error = lis_get_pipe(&fp0, &fp1)) < 0)
		goto no_pipe;
...
static inline void
lis_fifo_wait_for_partner(struct inode *i, unsigned int *cnt)
{
	int cur = *cnt;
...
static inline void
lis_fifo_wake_up_partner(struct inode *i)
{
	lis_wake_up_interruptible(PIPE_WAIT(*i));
}

static struct inode *
lis_fifo_info_new(struct inode *i)
{
...
lis_fifo_open_sync(struct inode *i, struct file *f)
{
	...
	if (!i || !f) {
		printk("fifo_open_sync(i@0x%p/%d,f@0x%p/%d)#%ld - NULL PARM!\n", i,
		       (i ? I_COUNT(i) : 0), f, (f ? F_COUNT(f) : 0), this_open);
		return (-EINVAL);
	}
	...
	if (LIS_DEBUG_VOPEN)
		printk("lis_fifo_open_sync(i@0x%p/%d,f@0x%p/%d)#%ld"
		       ...
		       this_open, head ? head->sd_name : "No-Strm", PIPE_READERS(*i),
		       PIPE_WRITERS(*i));
	...
	if (LIS_DEBUG_VOPEN)
		printk("lis_fifo_open_sync(i@0x%p/%d,f@0x%p/%d)#%ld \"%s\"" " >> error(%d)\n", i,
		       (i ? I_COUNT(i) : 0), f, (f ? F_COUNT(f) : 0), this_open, head->sd_name,
		       ret);
...
lis_fifo_close_sync(struct inode *i, struct file *f)
{
	...
	if (LIS_DEBUG_VCLOSE)
		printk("lis_fifo_close_sync(i@0x%p/%d,f@0x%p/%d)#%ld"
		       " \"%s\" >> %d reader(s) %d writer(s)\n", i, I_COUNT(i), f,
		       (f ? F_COUNT(f) : 0), this_close, (head
							  && head->sd_name ? head->sd_name : ""),
...
lis_fattach(struct file *f, const char *path)
{
		...
		if (result < 0) {
			...
			if (LIS_DEBUG_FATTACH || LIS_DEBUG_ADDRS || LIS_DEBUG_REFCNTS) {
				printk("lis_fattach(...) data @ 0x%p:\n"
				       ...
				       F_COUNT(data->file), data->head, data->path, data->sb);
	...
	if (++twice <= 2)
		printk("\nfattach is no longer implemented in kernels older " "than 2.4.7\n\n");
	return (-ENOSYS);
#endif				/* FATTACH_VIA_MOUNT */
}
...
lis_fdetach_stream(stdata_t *head)
{
	...
	while (n-- && !list_empty(&lis_fattaches)) {
		lis_fattach_t *data = list_entry(lis_fattaches.next, lis_fattach_t, list);

		if (data->head == head) {
			lis_spin_unlock(&lis_fattaches_lock);

			if (LIS_DEBUG_FATTACH || LIS_DEBUG_ADDRS) {
				printk("    fdetaching: " "data 0x%p head@0x%p \"%s\" sb 0x%p\n",
	...
	while (n-- && !list_empty(&lis_fattaches)) {
		...
		if (LIS_DEBUG_FATTACH || LIS_DEBUG_ADDRS)
			printk("    >> fdetaching data 0x%p head 0x%p sb 0x%p d 0x%p...\n", data,
			       data->head, data->sb, data->dentry);

		if ((error = lis_fdetach(data->path)) < 0) {
			if (LIS_DEBUG_FATTACH)
				printk("    >> fdetach data 0x%p failed (%d)\n", data, error);
		} else if (LIS_DEBUG_FATTACH)
...
static lis_fattach_t *
lis_fattach_new(struct file *f, const char *path)
{
	...
	if (LIS_DEBUG_FATTACH || LIS_DEBUG_REFCNTS) {
		printk("lis_fattach_new(f@0x%p/%d,\"%s\")" " => data@0x%p (%d/%d)\n", f, F_COUNT(f),
		       path, data, K_ATOMIC_READ(&num_fattaches_listed),
...
static void
lis_fattach_delete(lis_fattach_t * data)
{
	K_ATOMIC_DEC(&num_fattaches_allocd);

	if (LIS_DEBUG_FATTACH || LIS_DEBUG_REFCNTS) {
		printk("lis_fattach_delete(%p) (%d/%d)\n", data,
		       K_ATOMIC_READ(&num_fattaches_listed), K_ATOMIC_READ(&num_fattaches_allocd));
	}
...
static void
lis_fattach_insert(lis_fattach_t * data)
{
	lis_spin_lock(&lis_fattaches_lock);
...
static void
lis_fattach_remove(lis_fattach_t * data)
{
	lis_spin_lock(&lis_fattaches_lock);
	list_del(&data->list);
	lis_spin_unlock(&lis_fattaches_lock);
...
lis_sendfd(stdata_t *sendhd, unsigned int fd, struct file *fp)
{
	...
	error = -EPIPE;
	if (!sendhd || !(sendhd->magic == STDATA_MAGIC) || !(recvhd = sendhd->sd_peer)
	    || !(recvhd->magic == STDATA_MAGIC) || recvhd == sendhd
	    || F_ISSET(sendhd->sd_flag, STRHUP))
		goto not_fifo;

	error = -ENOSR;
	if (!(mp = allocb(sizeof(strrecvfd_t), BPRI_HI)))
		goto no_msg;
	...
	if (LIS_DEBUG_SNDFD || LIS_DEBUG_IOCTL || LIS_DEBUG_REFCNTS) {
		printk("lis_sendfd(...,%d,f@0x%p/%d) from \"%s\" to \"%s\"", fd, oldfp,
		       F_COUNT(oldfp), sendhd->sd_name, recvhd->sd_name);
		if (sendfd->r.fp)
			printk(" as f@0x%p/%d\n", fp, F_COUNT(fp));
		else
			printk("\n");
...
#if defined(KERNEL_2_5)
void
lis_tq_free_passfp(unsigned long arg)
#else
void
lis_tq_free_passfp(void *arg)
#endif
{
	mblk_t *mp;
	strrecvfd_t *sent;
	...
	while ((mp = lis_get_passfp()) != NULL) {
		...
		is_a_stream = (sent->f.fp->f_op == (&lis_streams_fops));
		 /**/ if (LIS_DEBUG_SNDFD || LIS_DEBUG_VCLOSE || LIS_DEBUG_REFCNTS) {
			...
			printk("lis_tq_free_passfp(m@0x%p)" " %s unreceived %sfile @0x%p/%d\n", mp,
			       (sent->r.fp ? "freeing" : "closing"),
			       (is_a_stream ? "STREAMS " : ""), f, (f ? F_COUNT(f) : 0));
			if (f) {
				struct dentry *d = f->f_dentry;
				struct inode *i = FILE_INODE(f);
				struct vfsmount *m = FILE_MNT(f);

				printk("    << d@0x%p/%d%s i@0x%p/%d%s", d, (d ? D_COUNT(d) : 0),
				       (d
					&& D_IS_LIS(d) ? " <LiS>" : ""), i, (i ? I_COUNT(i) : 0), (i
												   &&
												   I_IS_LIS
												   (i)
												   ?
...
lis_recvfd(stdata_t *recvhd, strrecvfd_t * recv, struct file *fp)
{
	...
	if (sent->r.fp && (sent->r.fp == fp)) {
		if (LIS_DEBUG_SNDFD | LIS_DEBUG_IOCTL || LIS_DEBUG_REFCNTS)
			printk("lis_recvfd(...,f@0x%p/%d) "
			       ...
			       (sent->f.fp ? F_COUNT(sent->f.fp) : 0), fp, (fp ? F_COUNT(fp) : 0),
			       sent->f.fp);
	...
	if (LIS_DEBUG_SNDFD || LIS_DEBUG_IOCTL || LIS_DEBUG_REFCNTS) {
		printk("lis_recvfd(...,f@0x%p/%d) as fd %d at \"%s\"\n", sent->f.fp,
		       (sent->f.fp ? F_COUNT(sent->f.fp) : 0), recv->f.fd, recvhd->sd_name);
	}
...
#else
	printk("lis_loadable_load: %s: " "kernel not compiled for dynamic module loading\n", name);
	return (-ENOSYS);
#endif
}
...
lis_ioctl32_str(unsigned int fd, unsigned int cmd, unsigned long arg)
{
	...
	if (copy_to_user((void *) &(ptr32->ic_timout), (void *) &(par64.ic_timout), sizeof(int))) {
...
static int
lis_write_qstats(char *buf, struct module_stat *ms)
{
	return sprintf(buf, "% 8d% 8d% 8d% 8d% 8d\n", lis_atomic_read(&ms->ms_pcnt),
		       lis_atomic_read(&ms->ms_scnt), lis_atomic_read(&ms->ms_ocnt),
		       lis_atomic_read(&ms->ms_ccnt), lis_atomic_read(&ms->ms_acnt));
}

static int
lis_write_mstats(char *buf, struct streamtab *st, int rq)
{
...
static int
lis_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	len +=
	    sprintf(page + len, "%18s: %8s %8s %8s %8s %8s\n", "driver/module", "#put", "#srvc",
		    "#open", "#close", "#admin");
...
lis_init_module(void)
{
	...
	/* Initialize every global variable to a default value, if there're modules w/ not-exported 
	   globals we should create init_() functions for them and call them from here. */
...

	{
		...
		if (err != 0) {
			printk("Linux Streams Subsystem failed to register its file system (%d).\n",
			       err);
			return (err);
		}
	}
	...
	printk("Linux STREAMS Subsystem ready.\n"
	       ...
	       "Using %s %s\n"
	       "==================================================================\n", lis_major,
	       lis_version, lis_date, lis_kernel_version, lis_poll_file, lis_stropts_file);
...
# endif
	...
	for (;;) {
		 ...
		 */
		if (lis_down(semp) < 0) {	/* sleep *//* interrupted */
		...
		if (cpu_id == 0 && lis_nxt_print_ptr != lis_print_buffer) {
			...
			else if (lis_secs() - seconds > 1) {	/* has enough time gone by? */
...
#define WORK_INCR	13
	...
	if (qsched_running) {	/* one of these is enough */
	...
	if (K_ATOMIC_READ(&lis_runq_cnt) == 0) {	/* no threads running */
		static int cnt;
	 ...
	 */
	if (req_cnt <= queue_load_thresh || queues_running == K_ATOMIC_READ(&lis_runq_cnt)
	    )
		goto return_point;
	 ...
	 * to be preempted.
	 */
	for (cpu = 0; cpu < K_ATOMIC_READ(&lis_runq_cnt); cpu++) {	/* find a sleeping thread */
		if (cpu != my_cpu	/* don't start on my cpu yet */
		    && !K_ATOMIC_READ(&lis_runq_active_flags[cpu])) {	/* sleeping thread */
...
************************************************************************/
void
lis_task_to_creds(lis_kcreds_t * cp)
{
#if 0
	int i;
	lis_flags_t psw;
...
void
lis_creds_to_task(lis_kcreds_t * cp)
{
#if 0
	int i;
	lis_flags_t psw;
...
void
lis_msgb_cache_freehdr(void *bp)
{
	if (lis_msgb_cachep) {
...
lis_kmem_cache_allochdr(void)
{
	...
	if ((!lis_max_msg_mem || K_ATOMIC_READ(&lis_strcount) < lis_max_msg_mem)
...
lis_cache_destroy(kmem_cache_t *p, lis_atomic_t *c, char *label)
{
	...
	if (n) {
		printk("lis_cache_destroy: " "Kernel cache \"%s\" has %d blocks still allocated\n"
		       "                   "
...
void *
lis_alloc_timer(char *file, int line)
{
	void *t;
