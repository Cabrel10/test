/*****************************************************************************
 ...
 @(#) lismem.c,v OpenSS7-0_9_2_D_rc2(1.1.1.2.4.5) 2005/12/19 03:22:20
...
#ident "@(#) lismem.c,v OpenSS7-0_9_2_D_rc2(1.1.1.2.4.5) 2005/12/19 03:22:20"
...
void
lis_mem_terminate(void)
{
	lis_slab_table_t *p;

	for (p = lis_slab_table; p->name != NULL; p++) {
		if (p->cache_struct == NULL)
			continue;
...
typedef struct contig_memlink {		/* 32 bytes long */
...
static contig_memlink_t *
find_entry(void *addr)
{
	contig_memlink_t *mp;

	for (mp = contig_mem_head; mp != NULL; mp = mp->link) {
		if (mp->page_addr == addr)
			return (mp);
	}
...
lis__kmalloc(int nbytes, int class, int use_cache)
{
	...
	else {			/* find appropriate slab */
		for (tp = lis_slab_table; tp->name != NULL; tp++) {
			if (tp->kflags == class && tp->cache_struct != NULL && nbytes <= tp->size)
				break;
		}

		if (tp->cache_struct == NULL)
			return (NULL);
