/*****************************************************************************
 ...
 @(#) lislocks.c,v OpenSS7-0_9_2_D_rc2(1.1.1.6.4.5) 2006/04/03 10:55:11
 ...
 Last Modified 2006/04/03 10:55:11 by brian

 *****************************************************************************/

#ident "@(#) lislocks.c,v OpenSS7-0_9_2_D_rc2(1.1.1.6.4.5) 2006/04/03 10:55:11"
...
************************************************************************/
static void
track_lock_contention(lis_spin_lock_t *lock)
{
...
static void
purge_lock_contention(lis_spin_lock_t *lock)
{
	lis_spin_lock_t **p = lis_lock_contention_table;
	unsigned long psw;
...
lis_lock_contention_msg(void)
{
	...
	for (; p != &lis_lock_contention_table[LOCK_CONTENTION_SIZE - 1]; p++) {
		...
		for (p2 = p + 1; p2 != &lis_lock_contention_table[LOCK_CONTENTION_SIZE]; p2++) {
			if ((*p2) == NULL)
				continue;
	...
	for (p = lis_lock_contention_table; p != &lis_lock_contention_table[LOCK_CONTENTION_SIZE];
	     p++) {
		if ((*p) == NULL)
			continue;

		sprintf(mp->b_wptr, "%10u(%d%%): %s last-owner: %s %d\n", (*p)->contention_cnt,
			tot_contention ? ((*p)->contention_cnt * 100) / tot_contention : 0,
			(*p)->name ? (*p)->name : "Anonymous",
			(*p)->owner_file ? (*p)->owner_file : "Unknown", (*p)->owner_line);
		while (*mp->b_wptr)
			mp->b_wptr++;
	}
...
************************************************************************/
static void
track_sem_contention(lis_semaphore_t *lsem)
{
...
static void
purge_sem_contention(lis_semaphore_t *lsem)
{
	lis_semaphore_t **p = lis_sem_contention_table;
	unsigned long psw;
...
lis_sem_contention_msg(void)
{
	...
	for (; p != &lis_sem_contention_table[LOCK_CONTENTION_SIZE - 1]; p++) {
		...
		for (p2 = p + 1; p2 != &lis_sem_contention_table[LOCK_CONTENTION_SIZE]; p2++) {
			if ((*p2) == NULL)
				continue;
	...
	for (p = lis_sem_contention_table; p != &lis_sem_contention_table[LOCK_CONTENTION_SIZE];
	     p++) {
		if ((*p) == NULL)
			continue;
...
static void
track_wakup_time(lis_semaphore_t *lsem)
{
	...
	do_gettimeofday(&x);
	interval =
	    (x.tv_sec * 1000000 + x.tv_usec) - (lsem->up_time.tv_sec * 1000000 +
						lsem->up_time.tv_usec);
	...
	for (p = lis_spl_track_ptr, n = SPL_TRACK_SIZE; n--;) {
		if (p->type != 0) {
			...
			printk("%u:%s State=%d "
			       ...
			       "Flgs=%03lx "
#endif
			       "%s #%u\n", p->cntr, typep, p->state, p->flags & 0x200, p->file,
			       p->line);
		}
...
lis_spin_lock_fcn(lis_spin_lock_t *lock, FL)
{
	...
	SAVE_FLAGS(prev);
	SET_SPINNER if ((tp = (void *) current) != lock->taskp) {
		if (spin_is_locked(l)) {
...
lis_spin_lock_irq_fcn(lis_spin_lock_t *lock, FL)
{
	...
	SET_SPINNER if ((tp = (void *) current) != THELOCK->taskp) {
		if (spin_is_locked(l)) {
			K_ATOMIC_INC(&lis_spin_lock_contention_count);
		SET_LOCK_CONTENTION}
		spin_lock_irq(l);
...
lis_spin_unlock_irq_fcn(lis_spin_lock_t *lock, FL)
{
	...
	LOCK_EXIT(lock, TRACK_UNLOCK, file, line, prev)
		...
		if (K_ATOMIC_READ(&THELOCK->nest) == 0) {
			THELOCK->taskp = NULL;
			SET_SPIN_UNLOCK spin_unlock_irq(l);
		}
	}
}
...
lis_spin_unlock_irqrestore_fcn(lis_spin_lock_t *lock, lis_flags_t * flags, FL)
{
	...
	LOCK_EXIT(lock, TRACK_UNLOCK, file, line, prev)
		...
		if (K_ATOMIC_READ(&THELOCK->nest) == 0) {
			THELOCK->taskp = NULL;
			SET_SPIN_UNLOCK spin_unlock_irqrestore(l, (*flags));
		}
	}
}
...
#if defined(CONFIG_DEV)
static void
lis_spin_lock_fill(lis_spin_lock_t *lock, const char *name, FL)
#else
static void
lis_spin_lock_fill(lis_spin_lock_t *lock, const char *name)
#endif
{
	DCL_l;
...
lis_spin_lock_alloc_fcn(const char *name, FL)
{
	...
	lock_size = sizeof(*lock) - sizeof(lock->spin_lock_mem) + sizeof(spinlock_t);
...
lis_rw_read_lock_irqsave_fcn(lis_rw_lock_t *lock, lis_flags_t * flags, FL)
{
	...
	SET_SPINNER read_lock_irqsave(r, (*flags));
...
lis_rw_write_lock_irqsave_fcn(lis_rw_lock_t *lock, lis_flags_t * flags, FL)
{
	...
	SET_SPINNER write_lock_irqsave(r, (*flags));
...
lis_rw_write_unlock_irqrestore_fcn(lis_rw_lock_t *lock, lis_flags_t * flags, FL)
{
	...
	LOCK_EXIT(lock, TRACK_UNLOCK, file, line, prev)
	    THELOCK->taskp = NULL;
	SET_SPIN_UNLOCK write_unlock_irqrestore(r, (*flags));
}
...
#else
static void
lis_rw_lock_fill(lis_rw_lock_t *lock, const char *name)
#endif
{
	DCL_r;
...
lis_rw_lock_alloc_fcn(const char *name, FL)
{
	...
	lock_size = sizeof(*lock) - sizeof(lock->rw_lock_mem) + sizeof(rwlock_t);
...
lis_down_fcn(lis_semaphore_t *lsem, FL)
{
	...
	if ((ret = down_trylock(sem))) {
		...
		SET_SEM_CONTENTION ret = down_interruptible(sem);
	}
...
static void
lis_sem_fill(lis_semaphore_t *lsem, int count)
{
	struct semaphore *sem = (struct semaphore *) lsem->sem_mem;
