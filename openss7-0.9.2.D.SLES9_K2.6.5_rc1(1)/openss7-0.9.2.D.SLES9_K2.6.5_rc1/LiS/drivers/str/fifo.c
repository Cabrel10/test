/*****************************************************************************
 ...
 @(#) fifo.c,v OpenSS7-0_9_2_D_rc2(1.1.1.2.4.9) 2005/12/19 03:22:17
 ...
 Last Modified 2005/12/19 03:22:17 by brian

 *****************************************************************************/

#ident "@(#) fifo.c,v OpenSS7-0_9_2_D_rc2(1.1.1.2.4.9) 2005/12/19 03:22:17"
...
static dev_t
fifo_mkdev(int unit_index)
{
	int maxminors = FIFO_MAX_MINOR + 1;

	return (makedevice(fifo__0_majors[(unit_index / maxminors)], unit_index % maxminors));
}
...
#ifdef FIFO_DEBUG
	cmn_err(CE_CONT, "%s_open( 0x%p, 0x%x, ... ) unit %d OK\n", MOD_NAME, q, *devp, m);
#endif
...
#ifdef FIFO_DEBUG
	cmn_err(CE_CONT, "%s_close( 0x%p, 0x%x, ... ) unit %d\n", MOD_NAME, q, flag,
		fdp ? fdp->index : 0);
#endif
 ...
 */
static void
fifo_wput(queue_t *q, mblk_t *mp)
{
	fifo_dev_t *fdp = (fifo_dev_t *) q->q_ptr;

#ifdef FIFO_DEBUG
	cmn_err(CE_CONT, "%s_wput( 0x%08x, 0x%08x ) unit %d\n", MOD_NAME, q, mp, fdp->index);
#endif
...
#ifdef FIFO_DEBUG
	cmn_err(CE_CONT, "%s_wsrv( 0x%08x ) unit %d\n", MOD_NAME, q, fdp->index);
#endif
 ...
 */
static void
fifo_rput(queue_t *q, mblk_t *mp)
{
	fifo_dev_t *fdp = (fifo_dev_t *) q->q_ptr;

#ifdef FIFO_DEBUG
	cmn_err(CE_CONT, "%s_rput( 0x%08x, 0x%08x ) unit %d\n", MOD_NAME, q, mp, fdp->index);
#endif
...
#endif
{
	...
	if (ret < 0) {
		cmn_err(CE_CONT, "%s - unable to register driver.\n", MOD_NAME);
		return ret;
	}
...
#endif
{
	...
	if (lis_unregister_strdev(FIFO__CMAJOR_0) < 0)
		cmn_err(CE_CONT, "%s - unable to unregister driver.\n", MOD_NAME);
	return;
}
