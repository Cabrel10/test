/*****************************************************************************
 ...
 @(#) head.c,v OpenSS7-0_9_2_D_rc2(1.1.1.12.4.14) 2006/07/07 21:21:47
 ...
 head.c,v
 Revision 1.1.1.12.4.14  2006/07/07 21:21:47  brian
...
#ident "@(#) head.c,v OpenSS7-0_9_2_D_rc2(1.1.1.12.4.14) 2006/07/07 21:21:47"
 ...
 * RCS Id          : head.c,v 1.1.1.12.4.14 2006/07/07 21:21:47 brian Exp
	...
	if (LIS_DEBUG_OPEN || LIS_DEBUG_CLOSE || LIS_DEBUG_REFCNTS) {
		if (!hd)
			...
			printk("lis_head_get(NULL) >> h@0x%p/++%d/%d \"%s\" {%s@%d}\n",
			       hd, LIS_SD_REFCNT(hd), LIS_SD_OPENCNT(hd), hd->sd_name, file, line);
		else
...
#endif
{
	...
	if (hd) {
		...
		if (LIS_DEBUG_OPEN || LIS_DEBUG_CLOSE || LIS_DEBUG_REFCNTS || oldcnt <= 0)
			printk("lis_head_put(h@0x%p/%s%d/%d) \"%s\" {%s@%d}\n",
			       hd, (oldcnt > 0 ? "--" : ""), LIS_SD_REFCNT(hd),
			       LIS_SD_OPENCNT(hd), hd->sd_name, file, line);

		if (tear_down) {
			lis_tear_down_stream(hd);
			hd = NULL;
		}
	} else if (LIS_DEBUG_OPEN || LIS_DEBUG_CLOSE)
...
new_muxid(void)
{
	...
	for (;;) {
		while ((my_id = lis_incr(&muxid)) < 0)
			muxid = 0;	/* keep positive */
		...
		for (hd = lis_stdata_head, n = K_ATOMIC_READ(&lis_stdata_cnt);
		     n > 0 || hd != lis_stdata_head; hd = hd->sd_next, n--) {
			if (hd->sd_mux.mx_index == my_id)
				break;	/* in-use */
		}
...
lis_i_unlink(struct inode *i, struct file *f, stdata_t *hd, int l_index, int cmd, cred_t *creds)
{
	...
	CLOCKON();
	for (hp = hd->sd_mux.mx_hd, prev = NULL, next = NULL; hp != NULL; hp = next) {
		...
		if ((err = lis_strdoioctl(f, hd, &ioc, creds, 0)) < 0 && cmd == I_UNLINK
			...
			if (l_index < 0) {	/* OK, skip this one *//* lose the memory?? */
				prev = hp;
				continue;
			}
		...
		if (LIS_SD_OPENCNT(hp) == 1) {	/* about to close */
			...
			CLOCKON();
		} else if (LIS_SD_OPENCNT(hp) > 1) {	/* be safe */
...
#define	RTN(v) do { CLOCKOFF(IOCTLTIME) ; return(v); } while (0)
	...
	lis_setq(muxed->sd_rq, hd->sd_strtab->st_muxrinit, hd->sd_strtab->st_muxwinit);
...
copyin_msg(struct file *f, stdata_t *hd, mblk_t **m, strbuf_t * uctl, strbuf_t * udat)
{
	...
	*m = NULL;		/* initially */
	...
	else if (*m == NULL)	/* have no ctl msg */
...
copyout_msgpart(struct file *f, mblk_t *m, strbuf_t * sb, strbuf_t * usb, int is_ctl, int doit)
{
	...
	if (doit) {		/* usb points to user space *//* ensure writeable */
	...
	for (rtnlen = 0; m != NULL; m = m->b_cont) {
		chunk = 0;
		mtype = lis_btype(m);
	...
	} else if (mtype >= 0)	/* saw at least one msg block */
...
lis_qdetach(queue_t *q, int do_close, int flag, cred_t *creds)
{
	...
	if (nwq_locked)
		LIS_QISRUNLOCK(OTHER(next_q), &psw_nwq);
	if (nrq_locked)
		LIS_QISRUNLOCK(next_q, &psw_nrq);
	if (pwq_locked)
		LIS_QISRUNLOCK(WR(prev_q), &psw_pwq);
	if (prq_locked)
		LIS_QISRUNLOCK(prev_q, &psw_prq);
...
lis_qopen(queue_t *q, dev_t *devp, int flag, cred_t *credp)
{
	...
	if (!(rq->q_qinfo->qi_qopen))
		return (0);	/* open not required */
...
lis_qattach(stdata_t *head, struct streamtab *info, dev_t *devp, int flags, cred_t *credp)
{
	...
	/* Set up the queue interface stuff. Do this before SPL to avoid nested SPLs. */
	if (info != NULL)
...
push_mod(stdata_t *head, streamtab_t *mod, int id, dev_t *devp, int flags, cred_t *creds)
{
	...
	if ((err = lis_set_q_sync(head->sd_wq->q_next, lis_fmod_sw[id].f_qlock_option)) < 0)
		goto return_error;
...
set_options(stdata_t *hd, stroptions_t * so)
{
	...
	if (F_ISSET(so->so_flags, SO_MREADOFF))
		CLR_SD_FLAG(hd, SNDMREAD);
	if (F_ISSET(so->so_flags, SO_NDELON))
		SET_SD_FLAG(hd, OLDNDELAY);
	if (F_ISSET(so->so_flags, SO_NDELOFF))
		CLR_SD_FLAG(hd, OLDNDELAY);
	if (F_ISSET(so->so_flags, SO_ISTTY))
		SET_SD_FLAG(hd, STRISTTY);
	if (F_ISSET(so->so_flags, SO_ISNTTY))
		CLR_SD_FLAG(hd, STRISTTY);
	if (F_ISSET(so->so_flags, SO_TOSTOP))
		SET_SD_FLAG(hd, STRTOSTOP);
	if (F_ISSET(so->so_flags, SO_TONSTOP))
		CLR_SD_FLAG(hd, STRTOSTOP);
	if (F_ISSET(so->so_flags, SO_STRHOLD))
		SET_SD_FLAG(hd, STRHOLD);
	if (F_ISSET(so->so_flags, SO_DELIM))
		SET_SD_FLAG(hd, STRDELIM);
	if (F_ISSET(so->so_flags, SO_NODELIM))
		CLR_SD_FLAG(hd, STRDELIM);
	...
	if (F_ISSET(so->so_flags, SO_BAND)) {
		if (F_ISSET(so->so_flags, SO_HIWAT))
			lis_strqset(hd->sd_rq, (qfields_t) QHIWAT, so->so_band, so->so_hiwat);
		if (F_ISSET(so->so_flags, SO_LOWAT))
			lis_strqset(hd->sd_rq, (qfields_t) QLOWAT, so->so_band, so->so_lowat);
...
lis_wake_up_poll(stdata_t *hd, int ev)
{
	...
	for (pd = hd->sd_polllist.ph_list; pd != NULL; pd = pd->pd_next) {
		...
		if (pd->pd_fn != NULL)	/* careful */
			(*pd->pd_fn) (pd->pd_arg);	/* this will wake up the polling process. */
	}
...
open_mods(stdata_t *head, dev_t *devp, int flags, cred_t *creds)
{
	...
	while (wq) {
		if ((err = lis_qopen(RD(wq), devp, flags, creds)) < 0)
			return (err);

		if (!SAMESTR(wq))
			return (0);

		if (!DEV_SAME(odev, *devp))
			return (0);
...
lis_process_rput(stdata_t *shead, queue_t *q, mblk_t *mp)
{
	...
	case M_PASSFP:
		...
		if (typ == M_PCPROTO || mp == q->q_first) {	/* hipri msg or 1st msg in queue */
			short evs = 0;
			short polls = 0;
...
lis_strwsrv(queue_t *q)
{
	...
	if (hd->magic != STDATA_MAGIC || hd->sd_wq != q)
		return (0);
...
lis_do_rd_tmout(unsigned long arg)
{
	...
	if (hd->magic != STDATA_MAGIC) {
		lis_error(LIS_ERROR, "lis_do_rd_tmout", "timeout with bad stream 0x%p", hd);
		return;
	}
...
#define RTN(v)	do { errv=(v); goto return_point; } while (0)
	...
	if (ioc->ic_len) {
		...
		if ((mdta = allocb(ioc->ic_len == TRANSPARENT ? sizeof(ioc->ic_dp)
		...
		} else if (ioc->ic_len != TRANSPARENT) {
	...
	case M_IOCACK:
...

		{
			...
			if (do_copyin) {
				...
				} else /* no data to return */ if (dat)	/* however, have empty bfr */
	      ...
	      cred_t *creds)
{
	...
	if (LIS_DEV_IS_CLONE(maj))
		lis_setq(head->sd_rq, clone_info.st_rdinit, clone_info.st_wrinit);
	else
...
lis_freezestr(queue_t *q)
{
	...
	if ((hd_peer = hd->sd_peer) && hd_peer != hd && !F_ISSET(hd_peer->sd_flag, STRFROZEN)) {
...
lis_stropen(struct inode *i, struct file *f)
{
	...
	CP(head, odev);
	err = 0;
	if (!existing_head) {	/* opening a new stream */
...
#if defined(LINUX)
		lis_cleanup_file_opening(f, head, err, oldd, oldd_cnt, oldmnt, oldmnt_cnt);
#else
...
#define RTN(e)  do { err=(e); goto return_point; } while (0)
	...
	if (ulen == LIS_GETMSG_PUTMSG_ULEN) {
		...
		if (!(err = lis_check_umem(fp, VERIFY_READ, ubuff, sizeof(args)))
			...
			err = lis_strputpmsg(i, fp, args.ctl, args.dat, args.band, args.flags);
		return (err);
	}
#endif
	...
	if ((err = lis_check_umem(fp, VERIFY_READ, ubuff, ulen)) < 0) {
		if (LIS_DEBUG_WRITE)
			printk("strwrite: stream %s: user address 0x%lx cannot be read from\n",
	...
	do {
		if (F_ISSET(hd->sd_flag, STWRERR)) {
			if (LIS_DEBUG_WRITE)
				printk("strwrite: stream %s: stream has received M_ERROR %d\n",
		...
		if (K_ATOMIC_READ(&hd->sd_wrcnt) != 0 || !lis_bcanputnext(hd->sd_wq, 0)
		...
		if (newmsg) {
			held->b_rptr = held->b_wptr = held->b_datap->db_base + hd->sd_wroff;
		...
		if (LIS_DEBUG_WRITE)
			printk("strwrite: stream %s: send %d bytes downstream to %s\n",
...
lis_requeue(stdata_t *hd, mblk_t *mp)
{
	...
	if (lis_btype(mp) == M_PCPROTO) {
		...
		hdmp = hd_rq->q_first;
		if (hdmp != NULL && lis_btype(hdmp) == M_PCPROTO) {	/* delete msg at head of q */
			hdmp = lis_getq(hd_rq);
			lis_freemsg(hdmp);
			LisDownCount(MSGQDSTRHD);	/* one fewer msg queued */
		}
...
#define	RTN(val)  do { err = val; goto return_point; } while (0)
	...
	if (ulen == LIS_GETMSG_PUTMSG_ULEN) {
		...
		if (!(err = lis_check_umem(fp, VERIFY_READ, ubuff, sizeof(args)))
			...
			err = lis_strgetpmsg(i, fp, args.ctl, args.dat, args.bandp, args.flagsp, 1);
		return (err);
	}
#endif
	...
	if ((err = lis_check_umem(fp, VERIFY_WRITE, ubuff, ulen)) < 0) {
		if (LIS_DEBUG_READ)
			printk("strread: stream %s: user buffer at 0x%lx cannot be written into\n",
			       hd->sd_name, (long) ubuff);
	...
	do {
		...
		while (hd_rq->q_first == NULL && msgs_read == 0) {	/* nothing to read */
			...
			if (!mread_sent && F_ISSET(hd->sd_flag, SNDMREAD)) {	/* only send once
				...
				qlocked = 0;
				if (LIS_DEBUG_READ)
					printk("strread: stream %s: send M_READ downstream to %s\n",
		...
		case M_PROTO:
			msgs_read++;
			if (LIS_DEBUG_READ)
				printk("strread: stream %s: RDOPT=0x%x", hd->sd_name, hd->sd_rdopt);
			if (F_ISSET(hd->sd_rdopt, RPROTNORM)) {	/* return with EBADMSG */
		...
		default:
			if (LIS_DEBUG_READ)
				printk("strread: stream %s: read %s: return EBADMSG\n",
				       hd->sd_name, lis_msg_type_name(mp));
			lis_freemsg(mp);
			RTN(-EBADMSG);
		}		/* sw db_type */
...
#define	RTN(val)  do { err = val; goto return_point; } while (0)
	...
	do {
		...
		couldput = (K_ATOMIC_READ(&hd->sd_wrcnt) == 0) &&
		    !F_ISSET(hd->sd_flag, STRFROZEN) &&
		    lis_bcanputnext(hd->sd_wq, (unsigned char) (band < 0 ? 0 : band));
		/* check the band */
	...
	if (LIS_DEBUG_PUTMSG) {
		printk("strputmsg: stream %s: send %d/%d ctl/data bytes downstream to %s\n",
		       hd->sd_name,
		       msg->b_datap->db_type == M_DATA ? 0 : lis_xmsgsize(msg),
		       lis_msgdsize(msg), lis_queue_name(hd->sd_wq->q_next));
...
#define RTN(v)	do { err=(v); goto err_return_point; } while (0)
	...
	rdq = hd->sd_rq;
	if (!LIS_CHECK_Q_MAGIC(rdq))
		RTN(-EINVAL);

	if (doit) {		/* fetch structs from usr space */
		if (ctl != NULL) {
	...
	if ((mp = rdq->q_first) == NULL) {	/* mp is 1st msg *//* empty read queue */
	...
	if (doit) {		/* it's get[p]msg() */
		...
		if (mtype == M_SIG) {	/* not suitable msg *//* msg still in queue */
			lis_check_m_sig(hd);	/* watch for M_SIG in q */
			err = -EAGAIN;
			goto return_point;
		}
		...
		if (lis_hipri(mtype))	/* M_PCPROTO */
			flags = MSG_HIPRI;
		else /* M_DATA or M_PROTO */ if (flags == RS_HIPRI) {	/* wants hi-priority only */
			err = -EBADMSG;
			goto return_point;	/* go return 'rtn' */
		} else
	...
	if (err == 0 && rtn >= 0) {	/* return flags to user */
		if (!doit || bandp == NULL) {	/* I_PEEK or getmsg *//* different encoding of flags */
			switch (flags) {
			case MSG_HIPRI:
				flags = RS_HIPRI;
				break;
	...
	} else /* getmsg or I_PEEK */ if (!doit && bandp != NULL)	/* kernel pointer */
		*bandp = band;
...
#define RTN(v)	do { err=(v); goto return_point; } while (0)
		...
		if (F_ISSET(hd->sd_flag, STRHUP | STRCLOSE))
			...
			if (LIS_DEBUG_IOCTL)
				printk("lis_strioctl(...,I_STR,...) "
				       ...
				       hd->sd_name, ioc.ic_cmd, ioc.ic_timout, ioc.ic_len, res);
	...
	case I_POP:		/* tested */
	{
		...
		if (LIS_DEBUG_IOCTL) {
			printk("lis_strioctl(...,I_POP,\"%s\") \"%s\" >> ", qname, hd->sd_name);
	...
	case I_SETSIG:
		if (!arg) {
			if (lis_del_from_elist(&(hd->sd_siglist), pid, S_ALL))
				err = -EINVAL;
			else if (!hd->sd_siglist)
				hd->sd_sigflags = 0;
		} else if (lis_add_to_elist(&(hd->sd_siglist), pid, (short) arg))
			err = -EAGAIN;
		else
			F_SET(hd->sd_sigflags, arg);

		if (LIS_DEBUG_IOCTL || LIS_DEBUG_SIG) {
			printk("lis_strioctl(...,I_SETSIG,%ld) \"%s\" >> ", arg, hd->sd_name);
			if (err)
				printk("error %d\n", err);
			else
				printk("OK\n");
	...
	case I_GETSIG:
	{
		...
		if (LIS_DEBUG_IOCTL)
			printk("lis_strioctl(...,I_GETSIG,...) \"%s\" >> ", hd->sd_name);

		err = lis_check_umem(f, VERIFY_WRITE, (char *) arg, sizeof(int));
		if (err == 0 && (evs = (int) lis_get_elist_ent(hd->sd_siglist, pid)) == 0)
			err = -EINVAL;
	...
	case I_FDINSERT:
	{
		...
		if ((err = lis_check_umem(f, VERIFY_READ, (char *) arg,
		    ...
		    || (err = lis_copyin(f, &strfdinsert, (char *) arg, sizeof(strfdinsert_t))) < 0)
			RTN(err);
		...
		ctl = (strbuf_t *) (arg + (unsigned long) &((strfdinsert_t *) NULL)->ctlbuf);
		dat = (strbuf_t *) (arg + (unsigned long) &((strfdinsert_t *) NULL)->databuf);
...
#endif
	case I_RECVFD:
		if ((err = lis_check_umem(f, VERIFY_WRITE, (char *) arg, sizeof(strrecvfd_t))) < 0) {
			RTN(err);
		} else {
			strrecvfd_t recv;
			...
			RTN(lis_copyout(f, (char *) &recv, (char *) arg, sizeof(strrecvfd_t)));
		}
		   ...
		   ioctl(fd,I_STR,{cmd,arg}) and pass down to strdoioctl() */
	{
		...
		if (LIS_DEBUG_IOCTL)
			printk("lis_strioctl(...) \"%s\""
			       ...
			       hd->sd_name, ioc.ic_cmd, ioc.ic_timout, res);
		RTN(res);
	}
	}			/* sw cmd */
...
#define	RTN(msk)	do { revents = msk; goto rtn_point; } while (0)
	...
	if ((mp = hd->sd_rq->q_first) != NULL) {	/* mp is 1st msg *//* a msg can be read */
		mtype = lis_btype(mp);
...
deschedule(queue_t *q)
{
	...
	if (F_ISSET(wq->q_flag, QSCAN)) {	/* remove from the scan list */
		LIS_QISRUNLOCK(wq, &psw1);
		CP(q, 0);
		p_prev = NULL;
		p_scan = (queue_t *) lis_scanqhead;
		while (p_scan != NULL) {	/* only wr q in scan list */
			if (!LIS_CHECK_Q_MAGIC(p_scan))
				break;

			if (p_scan == wq) {	/* p_scan points to our queue */
	...
	if ((wq->q_flag | rq->q_flag) & QENAB) {
		...
		while (p_scan != NULL) {
			...
			if (p_scan == rq || p_scan == wq) {	/* p_scan points to our queue */
				K_ATOMIC_DEC(&lis_runq_req_cnt);	/* one less thing to do */
				CP(p_scan, lis_qhead);
				CP(p_prev, lis_qtail);
				if (p_prev != NULL)	/* trailing ptr valid? */
...
close_action(stdata_t *head)
{
	...
	if (SAMESTR(head->sd_wq)) {	/* is there a "downstream" */
		...
		if (rslt < 0 && rslt != -EINTR)
			printk("lis_tear_down_stream: " "sleep_on_close_wt returned %d\n", rslt);
	}
...
lis_doclose(struct inode *i, struct file *f, stdata_t *head, cred_t *creds)
{
	...
	if ((LIS_DEBUG_CLOSE || LIS_DEBUG_ADDRS) || LIS_DEBUG_REFCNTS) {
		printk("lis_doclose(i@0x%p/%d,f@0x%p/%d,h@0x%p/%d/%d.%d,...)#%ld/%d\n"
		       ...
		       this_doclose, K_ATOMIC_READ(&lis_close_cnt), head->sd_name,
#if defined(LINUX) && defined(KERNEL_2_1)
...
#define RTNX	return(0)
	...
	if (!(head = FILE_STR(f))) {
		printk("lis_strclose: called with null stream (inode 0x%p file 0x%p)", i, f);
		CLOCKOFF(CLOSETIME);
		RTNX;
	}
